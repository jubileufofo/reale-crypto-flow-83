...
const Header = () => {
  ...
  return <>
      ...
      {/* Desktop/Tablet Header */}
      <header className={`fixed top-0 w-full z-50 transition-all duration-500 hidden md:block ${isScrolled ? 'bg-reale-black/80 backdrop-blur-xl border-b border-white/10' : ''}`}>
      ...
      <header className={`
          ...
          md:hidden
          ${isScrolled && showMobileHeader ? 'bg-reale-black/80 backdrop-blur-xl border-b border-white/10 translate-y-0 opacity-100' : 'translate-y-[-20px] opacity-0 pointer-events-none'}
        `}>
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <HamburgerButton className="text-white border-white/20 hover:bg-white/10" open={isDockOpen} setOpen={setIsDockOpen} />
        </div>
      </header>
